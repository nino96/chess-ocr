...
| Alternative browser board detector   | Classical computer vision and grid geometry  | Cheap diagnostic and fallback given only a small smoke screen; not a promotion contender     |
...
| Additional GB10 evaluation candidate | ChessQueries ViT-L/14 + square-query decoder | Unvalidated on printed pages; one bounded #4 screen under noncommercial terms, not a backend |
...
The dated decisions that produced this plan, and their supersessions, are in
[the decision log](docs/decisions/index.md). This plan states the outcome; the
log states when and why it was chosen.
